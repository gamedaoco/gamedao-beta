...
function Model(props) {
  const { scene, nodes, materials } = useGLTF('/DeLorean.glb')
...
function ModelFBX(){
    let fbx = useFBX('/CyberpunkDeLorean.fbx');
    //Add this line
...
export function ModelCar() {
  return (
    ...
    <Canvas dpr={[1, 2]} shadows camera={{ zoom: 5, fov: 45 }}>
      {/*
      ...
      */}
      <Suspense fallback={null}>
        ...
        <Stage environment={null} intensity={3} contactShadowOpacity={1} shadowBias={-0.0015}>
          <Model scale={0.001} />
        </Stage>
      </Suspense>
      {/* 
