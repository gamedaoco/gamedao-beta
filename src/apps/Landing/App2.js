...
function Scene({ dof }) {
  ...
  useFrame((state, delta) => {
    ...
    group.current.position.x = THREE.MathUtils.lerp(group.current.position.x, state.mouse.x * 20, 0.002)
  ...
  return (
    <group ref={group}>
      ...
      {layers.map(({ scale, texture, ref, factor = 0, scaleFactor = 1, wiggle = 0, z , x, y}, i) => (
        <Plane scale={scale} args={[1, 1, wiggle ? 10 : 1, wiggle ? 10 : 1]} position-z={z} position-x={x} position-y={y} key={i} ref={ref}>
